      ...
      {list.length === 0 ? (
        ...
        >
          {list.map((r) => {
            ...
            const titleText = toText(r.title, r.slug);
            ...
            return (
              ...
              >
                  ...
                  <div style={{ padding: 12 }}>
                    ...
                    {r.description && (
                      <div style={{ opacity: 0.8, fontSize: 14, marginTop: 4 }}>
                        {toText(r.description, titleText)}
                      </div>
                    )}
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 8 }}>
                      {tags.slice(0, 3).map((t: any, i: number) => {
                        const tagText = toText(t, `Etiqueta ${i + 1}`);
                        return (
                          <span
                            key={tagText || i}
                            style={{
                ...
                <button
                  type="button"
                  aria-label={isDeleting ? "Eliminando receta" : `Eliminar ${titleText}`}
                  onClick={(event) => {
